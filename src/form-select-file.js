...
export default function FormSelectFile() {
    ...
    const onClickButton = () => {
        ...
        if (inputFiles.current.files.length > maxNumFiles) {
            alert(`เลือกได้ไม่เกิน ${maxNumFiles} ไฟล์`);
            return;
        }
        for (let f of inputFiles.current.files) {
    ...
    return (
        <div className="mt-4 mx-auto p-3 rounded" style={{ width: "400px", background: "#cee" }}>
            <form>
                ...
                <div className="form-group mb-3">
                    <label htmlFor="maxNumSize" className="form-label">ขนาดไฟล์สูงสุด (KB)</label>
                    <select id="maxNumSize" className="form-select form-select-sm" ref={selectMaxSize}>
                        {maxFileSize.map(i => (
                            <option value={i} key={i}>{i}</option>
                        ))}
                    </select>
                ...
                <div className="form-group mb-3">
                    ...
                    <input type="file" id="file" accept="image/*" className="form-control form-control-sm" ref={inputFiles} multiple />
                </div>
